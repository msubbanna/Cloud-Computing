...
public class SharedMemory  {
  // public static final int InputFileLength = 1306122448;
	public static final int InputFileLength = 1367122448;
   public static final int chunkCount = 200;
   public static final int chunkSize = InputFileLength/chunkCount ;
   public static String chunkFileSize[] = new String [InputFileLength/chunkCount];
   ...
   static int threadCount = 2;
	...
	private static void chunking() throws IOException {
		...
		BufferedReader buffread = new BufferedReader(new FileReader("/data/input"));	
		String input = buffread.readLine(); 
		while(input != null) 
		{	
			FileWriter writer = new FileWriter(new File("/data" +"/part" +splitCount));
			int i=0;
			while(i < chunkSize)
			{
				writer.write(input);
	...
	public static void mergeFiles() throws IOException
	{
		...
		{
			buffread[i] = new BufferedReader(new FileReader(new File("/data"+"/part" +i)));			
			input[i] = buffread[i].readLine();
		}
		
		
		BufferedWriter writer = new BufferedWriter(new FileWriter(new File("/data" +"/sortedFile")));		
